...
router.get('/', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT id, judul, deskripsi, file, tanggal, created_at FROM tugas ORDER BY tanggal DESC, id DESC');
        res.json(rows);
    } catch (err) {
        console.error(err);
...
router.get('/:id', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT id, judul, deskripsi, file, tanggal FROM tugas WHERE id = ?', [req.params.id]);
